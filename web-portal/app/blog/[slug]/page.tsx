...
import { blogArticles, getBlogArticle } from "@/lib/all-blog-data";
...
export default async function BlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const structuredData = [
    {
      ...
      publisher: {
        ...
        logo: {
          "@type": "ImageObject",
          url: "https://academy.kydosdigital.com/brand/kydos-academy-logo-primary.svg"
        }
      }
    },
    {
  ...
  const otherArticles = blogArticles.filter((item) => item.slug !== article.slug && item.category !== article.category);
          ...
          <header className="blog-article-header">
            <div className="container blog-article-heading">
              <nav className="blog-breadcrumb" aria-label="Breadcrumb">
                <Link href="/">Kydos Academy</Link><span>›</span><Link href="/blog">Guides</Link><span>›</span><span>{article.category}</span>
              </nav>
              <div className="blog-meta">
                <span>{article.category}</span>
                <span>{article.readingTime}</span>
                <span>Updated {new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "long", year: "numeric" }).format(new Date(article.updated + "T12:00:00Z"))}</span>
              </div>
              <h1>{article.title}</h1>
            ...
            <div className="container blog-article-image-wrap">
              <img src={article.image} alt={article.imageAlt} className="blog-article-image" fetchPriority="high" />
            </div>
          </header>
                ...
                <aside className="blog-sources card">
                  ...
                  <ul>
                    {article.sources.map((source) => (
                      <li key={source.href}><a href={source.href} rel="nofollow">{source.label} ↗</a></li>
                    ))}
                  </ul>
                </aside>
              ) : null}
        ...
        <section className="section related-guides">
          <div className="container">
            <div className="portal-section-heading">
              <div><span className="eyebrow">Keep building</span><h2>Related agency guides</h2></div>
              <Link className="btn" href="/blog">All guides</Link>
            </div>
            <div className="related-blog-grid">
              {related.map((item) => (
                <article className="card" key={item.slug}>
                  ...
                  <p>{item.excerpt}</p>
                  <Link href={"/blog/" + item.slug}>Read guide →</Link>
                </article>
              ))}
            </div>
          </div>
