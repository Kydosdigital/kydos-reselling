...
export async function POST(request: Request) {
  ...
  if (session.payment_status !== "paid") {
    return Response.json({ received: true, provisioning: "payment_not_confirmed" });
  }
  ...
  if (!users.length) {
    return Response.json({ received: true, provisioning: "activation_required" });
  }
  ...
  await sql.query("update programme_orders set user_id = $1, provisioned_at = coalesce(provisioned_at, now()) where stripe_session_id = $2", [userId, session.id]);
  return Response.json({ received: true, provisioning: "existing_account_linked" });
}
